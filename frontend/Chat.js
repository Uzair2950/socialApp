...
//           // Check for auto-reply only if autoReplies is an array
//           if (Array.isArray(autoReplies)) {
//             const autoReply = autoReplies.find(
//               ar =>
//                 ar.message.toLowerCase() === newMessage.content.toLowerCase(),
//             );
//             if (autoReply) {
//               sendAutoReply(autoReply.reply);
//             }
...
//       const replies = response.data?.autoReplies || [];
//       setAutoReplies(replies);
//       console.log('Auto-replies loaded:', replies);
//     } catch (error) {
//       console.error('Error fetching auto-replies:', error);
//       // If there's an error, set empty array to avoid issues
//       setAutoReplies([]);
...
const Chat = ({navigation, route}) => {
  ...
  const fetchMessages = useCallback(
    async (pageNum = 0, isRefresh = false) => {
      try {
        ...
        if (response.data?.messages) {
          // Set the isGroup state from the response
    ...
    const handleNewMessage = async messageId => {
      try {
        ...
        if (response.data) {
          ...
          // Check for auto-reply only if it's not a group chat and autoReplies is an array
          if (!isGroup && Array.isArray(autoReplies) && otherParticipantData) {
            const autoReply = autoReplies.find(
              ar =>
                ar.message.toLowerCase() === newMessage.content.toLowerCase(),
            );
            if (autoReply) {
              sendAutoReply(autoReply.reply, otherParticipantData);
            }
          }
        }
      } catch (error) {
  ...
  // Fetch auto-replies from the other participant (only for non-group chats)
  const fetchAutoReplies = async () => {
    try {
      // If it's a group chat, don't fetch auto-replies
      if (isGroup) {
        console.log('Group chat detected, skipping auto-replies');
        setAutoReplies([]);
        return;
      }
      ...
      const userDataResponse = await axios.get(
        `${API_BASE_URL}/user/getUserData/${otherParticipantId}`,
      );

      setOtherParticipantData({
        _id: otherParticipantId,
        name: userDataResponse.data.name,
        imgUrl: userDataResponse.data.imgUrl,
      });
  ...
  // Send auto-reply with the other participant's info as sender
  const sendAutoReply = async (reply, participantData) => {
    try {
      const tempId = Date.now().toString();
      const newMessage = {
        _id: tempId,
        content: reply,
        senderId: participantData._id, // Use the other participant's ID
        createdAt: new Date().toISOString(),
        sentByMe: false, // Mark as not sent by me
        senderData: {
          name: participantData.name,
          imgUrl: participantData.imgUrl,
        },
        uniqueKey: `${tempId}_${participantData._id}`,
      };
      ...
      const response = await axios.post(
        `${API_BASE_URL}/chat/sendMessage/${chatId}`,
        {
          senderId: participantData._id, // Send as the other participant
          content: reply,
          isAutoReply: true, // Add flag to identify auto-replies
        },
      );
        ...
        prev.map(msg =>
          msg._id === tempId
            ? {
                ...msg,
                _id: response.data.message_id,
                uniqueKey: `${response.data.message_id}_${participantData._id}`,
              }
            : msg,
        ),
      );
      ...
      socket.current.emit('sendMessage', {
        chatId,
        messageId: response.data.message_id,
        senderId: participantData._id, // Emit as coming from the other participant
      });
    } catch (error) {
      console.error('Error sending auto-reply:', error);
    }
